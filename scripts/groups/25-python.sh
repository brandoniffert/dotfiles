...
group_python() {
  ...
  if ! [ -d "$pyenv_dir/versions/$PYTHON2_VERSION" ]; then
    PYENV_ROOT=$pyenv_dir /usr/local/bin/pyenv install $PYTHON2_VERSION
  ...
  if ! [ -d "$pyenv_dir/versions/$PYTHON3_VERSION" ]; then
    PYENV_ROOT=$pyenv_dir CFLAGS="-I$(/usr/local/bin/brew --prefix openssl)/include" LDFLAGS="-L$(/usr/local/bin/brew --prefix openssl)/lib" /usr/local/bin/pyenv install $PYTHON3_VERSION
  ...
  if ! PYENV_ROOT=$pyenv_dir /usr/local/bin/pyenv version | cut -d ' ' -f 1 | grep -Fxq $PYTHON3_VERSION; then
    PYENV_ROOT=$pyenv_dir /usr/local/bin/pyenv global $PYTHON3_VERSION
    PYENV_ROOT=$pyenv_dir /usr/local/bin/pyenv rehash
  ...
  if ! [ -d "$pyenv_dir/versions/py3nvim" ]; then
    PYENV_ROOT=$pyenv_dir /usr/local/bin/pyenv virtualenv $PYTHON3_VERSION py3nvim
